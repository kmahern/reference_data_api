...
def search_countries(search_term: str = Body(embed=True), skip: int = 0, limit: int = 1000, db: Session = Depends(get_db)):
    countries = crud.get_countries_by_search_term(search_term, db, skip=skip, limit=limit)
    countries_list = []
    for country in countries:
        countries_list.append( { "code": country.code, "description": country.description } )
